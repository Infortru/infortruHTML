...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if (!empty($imagen)) {
        ...
        $sql = "SELECT imagen FROM recetas WHERE id = ?";
    ...
    $sql = "UPDATE recetas SET titulo = ?, mensaje = ?, fecha = ?, imagen = ?, publicar = ? WHERE id = ?";
    $stmt = mysqli_prepare($link, $sql);
    mysqli_stmt_bind_param($stmt, 'sssssi', $titulo, $mensaje, $fecha, $imagen, $publicar, $id);

    if (mysqli_stmt_execute($stmt)) {
        echo "<h3>Receta actualizada correctamente.</h3>";
    } else {
        echo "Error al actualizar la receta: " . mysqli_error($link);
    }
} else {
